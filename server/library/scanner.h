...
private:
    ...
    void scan(const std::string& dir, int64_t parentId);
    void onFile(const std::string& filepath, int64_t parentId, std::vector<std::pair<std::vector<LibraryItem>, LibraryMetadata>>& items);
    bool checkAlbumArt(const std::string& directoryPath, std::string& hash);
    bool processAlbumArt(const std::string& filepath, const audio::AlbumArt& art, std::string& hash);
